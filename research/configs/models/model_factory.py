...
from research.configs.models import SVMModel, LogisticRegressionModel, NaiveBayesModel, MultinomialNaiveBayesModel, \
    ...
    LSTMModel, GRUModel, BiLSTMModel, CNNModel
...
class ModelFactory:
    ...
    # HuggingFace transformer models (tokenisation handled internally, raw-text interface)
    TRANSFORMER_MODELS = {'bert', 'roberta', 'fakebert', 'distilbert'}

    # PyTorch sequence models (also raw-text interface, but no HuggingFace dependency)
    DL_MODELS = {'lstm', 'gru', 'bilstm', 'cnn'}
    ...
    COMPATIBILITY_MAP = {
        ...
        # DL sequence models (word vectors are loaded internally)
        'lstm':   ['glove', 'word2vec'],
        'gru':    ['glove', 'word2vec'],
        'bilstm': ['glove', 'word2vec'],
        'cnn':    ['glove', 'word2vec'],

        # Transformer Models (embedding = pretrained checkpoint name)
    ...
    def is_transformer_model(model_type: str) -> bool:
        """Check if a model type is a HuggingFace transformer (raw-text interface)."""
        return model_type in ModelFactory.TRANSFORMER_MODELS

    @staticmethod
    def is_dl_model(model_type: str) -> bool:
        """Check if a model type is a PyTorch DL sequence model (raw-text interface)."""
        return model_type in ModelFactory.DL_MODELS

    @staticmethod
    def is_raw_text_model(model_type: str) -> bool:
        """Check if model operates on raw text (transformer or DL sequence model)."""
        return (
            model_type in ModelFactory.TRANSFORMER_MODELS
            or model_type in ModelFactory.DL_MODELS
        )
